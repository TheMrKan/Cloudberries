...
SYSTEM_PROMPT = (
    ...
    "Если запрос пользователя расплывчатый или не хватает деталей для поиска "
    "(например, просто 'нужен сервер' или 'хочу облачное хранилище') — "
    "НЕ вызывай search_services сразу. Задай 1-2 уточняющих вопроса: "
    "какая задача, нужны ли конкретные технологии, есть ли бюджет или требования по compliance.\n"
    "Если запрос содержит конкретику (категория сервиса + хотя бы одно требование) — "
    "используй search_services сразу.\n\n"
